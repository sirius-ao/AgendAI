...
test('páginas responsivas sem overflow, imagens carregadas e sem erros de JavaScript', async ({
  page,
}) => {
    ...
    for (const route of routes) {
      ...
      if (width === 1312 || width === 390) {
        await page.screenshot({
          path: `artifacts/screenshots/${route.slice(1) || 'inicio'}-${width}.png`,
          fullPage: true,
        });
      }
    }
...
test('planos mensal/anual, FAQ e comparação', async ({ page }) => {
  ...
  const pro = page
    ...
    .filter({ has: page.getByRole('heading', { name: 'Professor Pro', exact: true }) });
...
test('blog: categorias, pesquisa, paginação, artigos e newsletter honesta', async ({ page }) => {
  ...
  await page
    ...
    .getByRole('button', { name: 'Avaliações', exact: true })
    .click();
  ...
  await page
    .getByRole('searchbox', { name: 'Pesquisar artigos', exact: true })
    .fill('nao-existe-artigo');
